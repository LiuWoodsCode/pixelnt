...
Language=English
PixelNT could not be started as configured.  A previous working configuration was used instead.
.
Language=Russian
PixelNT could not be started as configured.  A previous working configuration was used instead.
.
Language=French
PixelNT n'a pas pu démarrer tel que configuré. Une précédente configuration fonctionnelle a été utilisé à la place.
.
Language=Polish
Nie można uruchomić systemu PixelNT zgodnie z aktualną konfiguracją. Zamiast niej użyto poprzedniej działającej konfiguracji.
.
Language=Romanian
PixelNT nu a putut fi pornit după configurație. În locul ei, a fost folosită o configurație funcțională anterioară.
.
...
PixelNT Management Console (MMC).
.
Language=Romanian
